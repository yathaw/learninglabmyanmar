$(document).ready(function(){
  ...
  var notificationsToggle    = $('.checkoutnoti');
     
  checkoutshowNoti();

  function checkoutshowNoti(){
    $.get("/checkoutnoti",function(response){
      var count = response.length;
      if(count > 0)
      {
        notificationsToggle.find('span').html(count);
        var html='';
        $('.checkoutpanelnoti').text(count+' New Notifications');
       function timeAgo(someDateInThePast) {
            ...
            html+='<a href="'+url+'" class="list-group-item">';
                   html+=`<div class="row g-0 align-items-center">
                      ...
                      <div class="col-10">
                        ...
                        <div class="text-muted small mt-1">${v.data.invoiceno}</div>
        ...
        $('#checkoutnoti_data').html(html);
        
      }else 
      {
        var html='';
        $('.checkoutpanelnoti').text('0 New Notifications');
  ...
  var channel = pusher.subscribe('checkout-channel');

  channel.bind('checkout-event', function(data) {
