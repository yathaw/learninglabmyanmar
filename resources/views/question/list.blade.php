<x-backend>
    ...
    @php  
        $defaultprofiile = asset('profiles/user.png'); 
    @endphp
	...
	<div class="row mb-2 mb-xl-3">
        ...
        <div class="col-6 mt-n1">
            ...
            <select class="form-select select2" id="courseId">
                <option> </option>
                <option value="0" data-id=0> All Courses </option>
                @foreach($courses as $course)
                    <option value="{{$course->id}}" data-id="{{ $course->id }}"> {{$course->title}}</option>
                @endforeach
...
<div id="questionDiv">
    ...
    <div class="row mt-3" id="emptyquestionDiv">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="row justify-content-center text-center">
                        ...
                        <div class="col-xl-6 col-lg-6 col-md-6 col-sm-10 col-10">
                            ...
                            <h3 class="mt-5"> No questions for you right now. </h3>

                            <p class="my-4 text-muted"> You don't have more questions to view! </p>
    ...
    @php
        $userprofile = $question->user->profile_photo_path;
					...
					<div class="row">
                        <div class="ribbon">
                            @if(count($question->answers) > 0)
                                <span class="ribbon2"><span> Already Answer </span></span>
                            @else
                                <span class="ribbon1"><span> Not Answer </span></span>

                            @endif
                        </div>

                        <div class="col-xl-2 col-lg-2 col-md-3 col-sm-12 col-12 d-flex align-items-center justify-content-center">
                            <img src="{{ asset($profile) }}" class="rounded-circle my-n1 img-fluid" alt="Avatar">
                        </div>
                        <div class="col-xl-10 col-lg-10 col-md-9 col-sm-12 col-12">
                            <h4> {{ $question->title }} <small class="text-success font-italic ml-3">by {{ $question->user->name }} </small></h4>

                            <p class=""> {{$question->created_at->diffForHumans()}}  </p>
                            
                            
                            <p> {!! Str::limit($question->description, 100) !!} </p>
                            
                            <a href="{{ route('backside.questions.show',$question->id) }}" class="btn btn-info mt-3 btn-sm"> <i class='align-middle' data-feather='eye'></i> View More </a>
                            <span class="badge rounded-pill bg-light text-dark float-right mt-3 px-3 py-2">  {{ $question->course->title }}  Course </span>
                        </div>
                    </div>
...
@section('script_content')
    ...
    <script type="text/javascript">
        // feather.replace();
        $('.select2').select2({
            placeholder: "<i class='align-middle' data-feather='filter'></i> Filter By Courses",
            theme: 'bootstrap4',
        ...
        $('#courseId').on("select2:select", function(e) {
            var courseid = $(this).children("option:selected").data('id');
            $.post("/getquestion_bycourseid",{id:courseid},function (res) {
                ...
                if(res.length > 0){
                    ...
                    $.each(res,function (i,v) {
                        ...
                        var userprofile = v.user.profile_photo_path;
                        var created_at = v.created_at;
                        var desc_xmlString = v.description

                        var date = moment(created_at).fromNow();

                        var desc_parse = $.parseHTML(desc_xmlString);
                        var description = desc_parse[0].innerHTML;


                        if (userprofile) {
                            var profile = userprofile;
                        }else{
                            var profile = `{{ $defaultprofiile }}`;
                        }

                        var url="{{route('backside.questions.show',':id')}}";
                        url=url.replace(':id',v.id);

                        html += `<div class="row"> <div class="col-12"> <div class="card">
                                <div class="card-header"> <div class="row"> `;

                        if (answers.length > 0) {
                            html += `<div class="ribbon">  <span class="ribbon2"><span> Already Answer </span></span> </div>`;
                        }else{
                            html += `<div class="ribbon"> <span class="ribbon1"><span> Not Answer </span></span> </div>`;
                        }

                        html += `<div class="col-xl-2 col-lg-2 col-md-3 col-sm-12 col-12 d-flex align-items-center justify-content-center">
                                <img src="${profile}" class="rounded-circle my-n1 img-fluid" alt="Avatar">
                            </div>`;

                        html +=`<div class="col-xl-10 col-lg-10 col-md-9 col-sm-12 col-12">
                                <h4> ${v.title} <small class="text-success font-italic ml-3">by ${v.user.name} </small></h4>
                                ...
                                <a href="${url}" class="btn btn-info mt-3 btn-sm"> <i class='align-middle' data-feather='eye'></i> View More </a>
                                <span class="badge rounded-pill bg-light text-dark float-right mt-3 px-3 py-2">  ${v.course.title}  Course </span>
                            </div>`;
